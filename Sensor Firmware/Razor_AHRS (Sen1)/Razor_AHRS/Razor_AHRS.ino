...
#define ACCEL_Z_MIN ((float) -237)
#define ACCEL_Z_MAX ((float) 273)

// "magn x,y,z (min/max) = -511.00/581.00  -516.00/568.00  -489.00/486.00"
#define MAGN_X_MIN ((float) -464)
#define MAGN_X_MAX ((float) 732)
#define MAGN_Y_MIN ((float) -649)
#define MAGN_Y_MAX ((float) 761)
#define MAGN_Z_MIN ((float) -576)
#define MAGN_Z_MAX ((float) 459)
...
#define GYRO_AVERAGE_OFFSET_X ((float) 5.42)
#define GYRO_AVERAGE_OFFSET_Y ((float) 2.83)
#define GYRO_AVERAGE_OFFSET_Z ((float) 4.77)
*/
...
//      if (output_stream_on || output_single_on) output_sensors();
      if( i < FHT_N )
      {
        fht_input[i] = (int)(sqrt(accel[0] * accel[0] + accel[1] * accel[1] + accel[2] * accel[2]));
        i++;
      }
      else
      {
        ...
        for (i = 0; i < 128; i = i + 1) 
        {
          Serial.print(i * 0.1953125); Serial.print(","); Serial.println(fht_log_out[i]);
        }
      }
    }
